...
export const publishCommand = new Command('publish')
  .description('Publish a new version of an existing skill to the local registry')
  .argument('<slug>', 'Skill slug')
  .option('-v, --version <version>', 'New version (must be higher than current)')
  ...
  .action(async (slug: string, options) => {
    try {
      ...
      if (!localRegistry.skillExists(slug)) {
        ...
        console.log(`Use ${chalk.cyan(`skills new ${slug}`)} to create it first.`);
        process.exit(1);
      }
      ...
      if (!isValidVersion(newVersion)) {
        console.log(chalk.red(`Error: Invalid version '${newVersion}'. Use semver format (e.g., 1.0.1).`));
        process.exit(1);
      }
      ...
      if (!isGreaterThan(newVersion, currentVersion)) {
        console.log(
          chalk.red(`Error: New version '${newVersion}' must be higher than current '${currentVersion}'.`)
        );
        process.exit(1);
      }
      ...
      if (localRegistry.versionExists(slug, newVersion)) {
        console.log(chalk.red(`Error: Version '${newVersion}' already exists.`));
        process.exit(1);
      }
      ...
      if (!currentSkill) {
        console.log(chalk.red(`Error: Could not read current content for '${slug}'.`));
        process.exit(1);
      }
      ...
      try {
        ...
        console.log(`  ${chalk.cyan(`skills sync`)}             Update projects using this skill`);
        console.log(`  ${chalk.cyan(`skills info ${slug}`)}     View all versions`);
      } catch (error) {
