...
export const deleteCommand = new Command('delete')
  ...
  .argument('<skill>', 'Skill to delete, format: <slug> (local) or <registry>/<slug> (remote)')
  .action(async (skill: string) => {
    try {
      ...
      if (parts.length === 1) {
        ...
        const spinner = ora(`Deleting ${chalk.cyan(slug)} from local registry...`).start();
        ...
        try {
          ...
          if (deleted) {
            ...
            console.log('');
            console.log(chalk.gray(`Note: You may also need to run ${chalk.cyan(`skills remove ${slug}`)} from any projects that use this skill.`));
          } else {
        ...
        try {
          ...
          spinner.succeed(`Deleted ${chalk.cyan(skillSlug)} from registry ${chalk.cyan(registrySlug)}`);
          console.log('');
          console.log(chalk.gray(`Note: You may also need to run ${chalk.cyan(`skills remove ${skillSlug}`)} from any projects that use this skill.`));
        } catch (error) {
          spinner.fail('Failed to delete skill');
          throw error;
        }
        ...
        console.log(`Use: ${chalk.cyan('skills delete <slug>')} (local) or ${chalk.cyan('skills delete <registry>/<slug>')} (remote)`);
        process.exit(1);
      }
    } catch (error) {
